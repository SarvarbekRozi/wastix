      ...
      <div class="row clearfix">
          ...
          <div v-for="(item, index) in items" :key="item.id"
            ...
            <div class="news-block-two compact-news-card wow fadeInUp animated"
              ...
              <div class="inner-box">
                <div class="image-box">
                  <figure class="image">
                    <a :href="`/announcements/${item.slug}`">
                ...
                <div class="lower-content">
                  <span class="category">{{ $t('page.e_lon_34') }}</span>
                  <h3><a :href="`/announcements/${item.slug}`">{{ item[`title_${$i18n.locale}`] }}</a></h3>
        ...
        <template v-else>
          <div class="col-12 text-center py-5">
            <p>{{ $t('page.e_lonlar_yuklanmoqda_35') }}</p>
          </div>
        </template>
      </div>
...
const { t } = useI18n()
useHead({ title: computed(() => `${t('nav.announcements')} - Trust Standart`) })
</script>
